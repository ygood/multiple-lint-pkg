...
| Git 规范                                                          | [commitlint](https://commitlint.js.org/#/)                 | [@commitlint/mlp-commitlint-config](https://www.npmjs.com/package/@commitlint/mlp-commitlint-config) |
...
- 写入各种配置文件，包括：
  ...
  - `commitlint.config.js`：commitlint 配置（继承 `@commitlint/mlp-commitlint-config`）
...
> 注 1：如果项目已经配置过 ESLint、stylelint 等 Linter，执行 `mlp-lint init` 将会提示存在冲突的依赖和配置，并在得到确认后进行覆盖：
...
`mlp-lint` 会读取执行目录下的 `mlp-lint.config.js` 作为配置文件。`mlp-lint init` 会在执行目录下新增如下的 `mlp-lint.config.js` 文件：
