...
import { MARKDOWN_LINT_FILE_EXT, MARKDOWN_LINT_IGNORE_PATTERN } from '../../utils/contans';
import getMarkdownlintConfig from './getMarkdownlintConfig';
import { formatMarkdownlintResults } from './formatMarkdownlintResults';

export interface DoMarkdownlintOptions extends IScanOptions {
  pkg: IPKG;
  config?: Config;
}
export default async (options: DoMarkdownlintOptions) => {
  let files: string[] = [];
  if (options.files) {
